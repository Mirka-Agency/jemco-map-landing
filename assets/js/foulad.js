	...
	function openRequestForm() {
		...
		function validate() {
			...
			if (!phone) {
				...
				// Normalize Persian and Arabic-Indic digits to ASCII digits so
				// users can enter تلفن using فارسی numerals (e.g. ۰۹۱۲...)
				...
				let p = toAscii(phone);
				// remove common separators/whitespace but keep leading + if present
				p = p.replace(/[\s\-()]/g, "");
		...
		form.addEventListener("submit", (ev) => {
			...
			const WEBHOOK_URL =
				"https://n8n.mirka.agency/webhook/landing-form-data";
